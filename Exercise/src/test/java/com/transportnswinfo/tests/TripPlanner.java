...
public class TripPlanner {
	...
	/*	//Clicking on options button
		WebElement options=driver.findElement(By.xpath("//div/journey-options/button[@type='button']"));
				options.click();
		...
		//Trip options document		
				...
				List<WebElement> modes=driver.findElements(By.cssSelector("div[class='tp-option-select checkbox checked']"));
						 //row tp-search-modes
				System.out.println("Modes list size:"+modes.size());
				System.out.println("Modes picked by default are: ");
				for(int i=0;i<modes.size();i++){
					WebElement we=modes.get(i);
					 String m=we.getText();
				...
				WebElement opal=driver.findElement(By.id("opalServicesSelectTickTitle-label"));
				if(opal.isSelected())
				{
					System.out.println("Opal Service: "+opal.getText());
				}
				else{
				...
				WebElement accessibility=driver.findElement(By.id("accessibilitySelectAccessibleIconTickTitle"));
				if(accessibility.isSelected())
				{
					System.out.println("Accessibility services : "+accessibility.getText());
				}else{
					System.out.println("Accessibility services are not opted");
				}
				
				
				//4.Walk speed section
				System.out.println("*********************Walk speed****************************");
				
				WebElement walk=driver.findElement(By.name("search-options-to-from-walk-speed"));
				...
				List<WebElement> prefer=driver.findElements(By.cssSelector("input[name='trip-options-trip-preference']"));
				...
				for(int j=0;j<prefer.size();j++){
					...
					if(TP.isSelected()){
						...
						System.out.println(TP.getText()+" option is:" +TP.getAttribute("checked"));
					}
...
//>>>>> when the done button of the form is clicked, it is closed, but the Travel info on the main menu tab is automatically cliked and 
 ///>>makes  it difficult to  look for Go button
		...
		*/
					...
					for(int s=0;s<TList.size();s++){
						...
						List<WebElement> Tnames=driver.findElements(By.xpath("//div/ul/li[@class='tripLegs ng-scope']"));
						System.out.println("The travel schedule: "+Tnames.size());
								
						for(int t=0;t<Tnames.size();t++)
							{
								String Tname=driver.findElement(By.xpath("//span[@class='tp_train-route-number-label']")).getText();
								System.out.println(Tname);
								System.out.println("**************************************************************************");
							
						
								List<WebElement> tpresult=driver.findElements(By.xpath("//div[@class='tp-result-item-timing']"));
	...
	List<WebElement>acc=driver.findElements(By.xpath("//li[@ng-if='tripSummaryVm.isAccessible'][@class='ng-scope']"));
	System.out.println("size:"+acc.size());
	for(int a =0;a<acc.size();a++){
		String acctext=driver.findElement(By.xpath("//span[@class='sr-only']")).getText();
		System.out.println(acctext);
		
		String opal1=driver.findElement(By.xpath("//span[@class='opal-fare ng-binding']")).getText();
		System.out.println(opal1);
